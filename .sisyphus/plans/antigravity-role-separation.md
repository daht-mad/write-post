...
### Definition of Done
- [ ] SKILL.md Antigravity 1차 방법에 다른 도구들과 동등한 수준의 역할 분리 지침이 있음
- [ ] 검색 결과에서 사용자 요청/AI 응답을 분리하는 구체적 방법이 명시되어 있음
- [ ] 총 줄 수 ≤ 500줄
...
- [ ] 1. Antigravity 1차 방법에 역할 분리 + 필터링 지침 추가
...
- [ ] 2. (선택) 정리 규칙에 Antigravity 1차 방법 필터링 항목 추가
...
### Final Checklist
- [ ] Antigravity 1차 방법에 역할 분리 지침 존재 (다른 도구와 동등 수준)
- [ ] 사용자 요청 → 코드블록, AI 작업 → 불릿 포인트 매핑 명시
- [ ] 폴백 전략 (역할 불명확 시) 명시
- [ ] AI 응답 전문/도구 로그/에이전트 질문 제외 명시
- [ ] `brain/` 보조 자료 역할 약화
- [ ] 날짜 정보 추출 지시 포함
- [ ] 2차 방법, Phase 1/2/3 워크플로우, 다른 도구 파싱 가이드 무변경
- [ ] 총 줄 수 ≤ 500
